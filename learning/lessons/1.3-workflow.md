# ⚠️ DEPRECATED: Lesson 1.3 - Workflow Skeleton & ADK Orchestration

**Status:** Part of OLD automated system (2024-12-14)
**Deprecated:** 2024-12-16 (Pivot to Interactive Partner)
**See instead:** `phase1-reboot.md` - No orchestrator in new system (Playground replaces it)
**Why kept:** Historical reference - shows what didn't work (automated coordination)
...
return {
    "status": "success",
    "path": str(draft_path)  # ← Convert Path to string
}
```
...
**Guidelines:**
- Use **generic names** when context is clear (e.g., `read_draft_tool` → `"content"`)
...
**Example:**
- ❌ `split_draft_tool(draft, outline)` - needs to understand semantic matching
- ✅ `content_splitter` agent - delegates understanding to LLM
